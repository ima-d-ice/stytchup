...
export default function SubmitRequirements() {
  ...
  const [form, setForm] = useState({ chest: '', waist: '', hips: '', height: '', unit: 'cm', notes: '' });

  const handleSubmit = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    try {
      const res = await fetch(`${API_URL}/orders/submit-measurements`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ orderId: id, measurements: form }),
      });
      if (res.ok) {
        navigate('/orders');
      } else {
    ...
    <div className="min-h-screen bg-[#FFFBEB] p-6 flex justify-center items-center font-sans">
      <div className="max-w-xl w-full bg-white rounded-3xl shadow-xl p-8 border-2 border-[#FFC629]">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Your payment is secure. Now, please provide your measurements so the designer can start working.</p>
        </div>
            ...
            <div className="bg-gray-100 p-1 rounded-full inline-flex">
              {['cm', 'inches'].map((unit) => (
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${form.unit === unit ? 'bg-black text-white shadow-md' : 'text-gray-500 hover:text-black'}`}
                >
