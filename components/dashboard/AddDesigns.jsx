...
export default function AddDesignPage() {
    ...
    return (<div className="max-w-xl mx-auto mt-10 p-8 bg-linear-to-br from-gray-50 to-gray-200 shadow-2xl rounded-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {feedback && (<div className={`p-3 rounded-md text-sm ${feedback.type === 'success'
        ...
        <div>
          ...
          <input type="text" required value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"/>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:space-x-4">
          ...
          <div className="flex-1">
            ...
            <input type="number" required value={price} onChange={(e) => setPrice(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"/>
          </div>

          {/* Category */}
          <div className="flex-1 mt-6 md:mt-0">
            <label className="block text-sm font-medium text-gray-700">Category</label>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
        ...
        <div className="border-2 border-dashed border-blue-300 rounded-lg p-6 text-center bg-blue-50/50">
          ...
          {imageUrl ? (<div className="relative inline-block">
              <Image src={imageUrl} alt="Preview" width={192} height={192} className="h-48 w-auto mx-auto rounded-md object-cover"/>
              <button type="button" onClick={() => setImageUrl("")} className="mt-2 text-sm text-red-600 hover:text-red-800 underline transition">
                Remove & Upload Different Image
              </button>
            </div>) : (<UploadButton endpoint="imageUploader" onClientUploadComplete={(res) => {
        ...
        {/* Submit Button */}
        <button type="submit" disabled={!imageUrl || isSubmitting} className={`w-full py-3 px-4 rounded-md text-white font-semibold transition-all duration-300
