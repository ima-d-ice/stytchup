...
export default function AvatarCard({ name, email, location, avatarUrl, onLocationChange, onAvatarChange }) {
    return (<div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex flex-col items-center text-center sticky top-8">
      ...
      <div className="relative w-32 h-32 mb-4 group">
        ...
        <div className="w-full h-full rounded-full bg-yellow-400 flex items-center justify-center border-4 border-white shadow-md overflow-hidden relative z-0">
          {avatarUrl ? (<Image src={avatarUrl} alt="Avatar" fill className="object-cover" sizes="(max-width: 768px) 100vw, 128px"/>) : (<span className="text-4xl font-bold text-white">
      ...
      <div className="w-full text-left">
        ...
        <input type="text" name="location" value={location} onChange={onLocationChange} placeholder="e.g. New York, USA" className="w-full bg-gray-50 rounded-xl px-4 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition-all border border-transparent focus:bg-white"/>
      </div>
    </div>);
}
