...
const TopDesigns = async () => {
    ...
    return (<section className="py-20 bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {designs.length > 0 ? (<div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {designs.map((design) => (<Link href={`/designs/${design.id}`} key={design.id} className="group relative">
                ...
                <div className="aspect-3/4 w-full overflow-hidden rounded-3xl bg-gray-200 shadow-sm transition-all duration-300 group-hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
                  <Image src={design.imageUrl} alt={design.title} fill className="h-full w-full object-cover object-center transition-transform duration-700 group-hover:scale-105"/>
                  {/* Quick Add overlay */}
          ...
          </div>) : (<div className="rounded-3xl border-2 border-dashed border-gray-300 p-12 text-center">
            <p className="text-gray-500">No designs found. Is the backend running?</p>
          </div>)}

        <div className="mt-12 text-center md:hidden">
          <Link href="/designs" className="inline-flex items-center px-8 py-3 text-base font-bold text-gray-900 bg-[#FFC629] rounded-full hover:bg-[#e5b225] transition-colors">
            Explore All Designs
          </Link>
        </div>
      </div>
