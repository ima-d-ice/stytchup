...
export default function MyOrdersPage() {
    ...
    const handleConfirmDelivery = async (orderId) => {
        if (!confirm("Please confirm you have received the physical package."))
            return;
        try {
    ...
    if (loading)
        return <div className="min-h-screen flex items-center justify-center bg-yellow-50/50">Loading orders...</div>;
    return (<div className="min-h-screen bg-white p-4 md:p-8 font-sans text-gray-900">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-black text-gray-900 mb-8 tracking-tight">Your Orders</h1>

        <div className="space-y-6">
          {orders.length === 0 ? (<div className="text-center py-24 bg-gray-50 rounded-4xl border-2 border-dashed border-gray-200">
              ...
              <Link href="/designs" className="inline-block bg-[#FFC629] text-black px-8 py-3 rounded-full font-bold shadow-lg hover:shadow-xl hover:bg-[#E5B225] transition-all transform hover:-translate-y-1">
            ...
            </div>) : (orders.map((order) => (<div key={order.id} className="bg-white rounded-4xl p-6 shadow-xl shadow-gray-100 border border-gray-100 flex flex-col md:flex-row gap-6 items-start transition hover:shadow-2xl hover:shadow-gray-200/50 duration-300">
                ...
                <div className="relative h-32 w-32 shrink-0 rounded-2xl overflow-hidden bg-gray-100 border border-gray-100 shadow-inner">
                  {order.designSnapshot?.image && (<Image src={order.designSnapshot.image} alt={order.designSnapshot.title || "Design"} fill className="object-cover"/>)}
                </div>
                ...
                <div className="flex-1 w-full flex flex-col justify-between">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-50">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        {order.status === 'AWAITING_REQUIREMENTS' && (<Link href={`/orders/${order.id}/submit-requirements`} className="inline-flex items-center px-6 py-2 bg-red-500 text-white rounded-full font-bold text-sm hover:bg-red-600 transition-all shadow-md hover:shadow-lg animate-pulse">
                            ⚠️ Submit Measurements
                            </Link>)}

                        {/* 2. If Shipped -> Show 'Confirm Receipt' button */}
                        {order.status === 'SHIPPED' && (<div className="flex items-center gap-4 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100">
                                ...
                                <button onClick={() => handleConfirmDelivery(order.id)} className="ml-2 px-5 py-2 bg-black text-white rounded-full font-bold text-xs hover:bg-gray-800 transition-all shadow-md">
                                    Confirm Receipt
                                </button>
                            </div>)}
                    </div>
...
function StatusBadge({ status }) {
    ...
    return (<span className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider shadow-sm ${styles[status] || "bg-gray-100"}`}>
