    ...
    return (<div className="flex flex-col h-screen bg-gray-50 font-sans">
            ...
            return (<div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] md:max-w-md p-4 rounded-2xl shadow-sm text-sm ${isMe ? 'bg-black text-white rounded-tr-none' : 'bg-white text-gray-900 rounded-tl-none'}`}>
                ...
                {msg.isOffer && (<div className={`mt-1 p-5 rounded-xl border-2 ${isMe ? 'bg-gray-800 border-gray-600' : 'bg-yellow-50 border-[#FFC629]'}`}>
                    ...
                    {msg.offerStatus === 'ACCEPTED' ? (<div className="w-full py-2 bg-green-500/20 text-green-500 text-center font-bold rounded-lg border border-green-500/50">
                         ✅ Accepted
                      </div>) : (!isMe && (<RazorpayButton amount={msg.offerPrice || 0} sourceId={msg.id} type="CHAT_OFFER" // 👈 UPDATED TYPE
                     buttonText="Accept & Pay" onSuccess={() => window.location.reload()}/>))}
      ...
      <div className="bg-white border-t p-4">
        <form onSubmit={handleSendMessage} className="flex gap-2 max-w-4xl mx-auto items-center">
          ...
          <input value={inputText} onChange={(e) => setInputText(e.target.value)} className="flex-1 text-gray-700 bg-gray-100 rounded-full px-5 py-3 outline-none focus:ring-2 focus:ring-[#FFC629] transition-all" placeholder="Type a message..."/>
          <button type="submit" className="bg-black text-white rounded-full p-3 hover:bg-gray-800">➤</button>
        </form>
      </div>

      {/* Offer Modal */}
      {showOfferModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
