      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 mt-4 lg:mt-8">
        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16">
          ...
          <div className="group relative aspect-3/4 w-full overflow-hidden rounded-3xl bg-gray-100 shadow-sm transition-all duration-300 hover:shadow-[0_8px_30px_rgb(255,198,41,0.2)]">
            <Image src={design.imageUrl} alt={design.title} fill className="h-full w-full object-cover object-center transform transition-transform duration-700 group-hover:scale-105" priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 600px"/>
             {/* Gradient Overlay */}
             ...
             <div className="absolute top-4 left-4">
                {design.type === 'CUSTOM' ? (<span className="inline-flex items-center rounded-full bg-black/80 backdrop-blur-md px-4 py-1.5 text-xs font-bold text-white tracking-wider shadow-lg">
                    CUSTOM REQUEST
                  </span>) : (<span className="inline-flex items-center rounded-full bg-[#FFC629] px-4 py-1.5 text-xs font-bold text-black tracking-wider shadow-lg">
                    MADE TO ORDER
                  </span>)}
             </div>
          </div>
        ...
        <RazorpayButton amount={design.price} sourceId={design.id} type="CATALOG" buttonText="Order Now (Made to Measure)"/>) : (
