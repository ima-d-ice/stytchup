...
// --- The Page Component ---
export default async function DesignerPortfolioPage({ params, }) {
    ...
    return (<div className="min-h-screen bg-gray-50 font-sans selection:bg-[#FFC629] selection:text-black">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden rounded-[2.5rem] bg-white shadow-sm ring-1 ring-gray-100 lg:grid lg:grid-cols-12 lg:gap-x-8">
          ...
          <div className="p-8 lg:col-span-5 xl:col-span-4 flex flex-col items-center text-center lg:items-start lg:text-left lg:border-r lg:border-gray-100">
            <div className="relative h-40 w-40 sm:h-48 sm:w-48 mb-6">
              {profile?.avatarUrl ? (<Image src={profile.avatarUrl} alt={name || 'Designer'} fill className="rounded-3xl object-cover shadow-lg border-4 border-white ring-2 ring-[#FFC629]"/>) : (<div className="flex h-full w-full items-center justify-center rounded-3xl bg-[#FFC629] text-6xl font-black text-white shadow-inner">
                  {initial}
                </div>)}
            </div>
            ...
            <div className="mt-6 w-full space-y-3">
              ...
              {profile?.socialLinks?.website && (<a href={profile.socialLinks.website} target="_blank" rel="noreferrer" className="flex w-full justify-center rounded-2xl border-2 border-gray-100 bg-white py-3 text-sm font-bold text-gray-600 transition hover:border-gray-200 hover:bg-gray-50">
                   Visit Website
                </a>)}
            </div>
          </div>
          ...
          <div className="bg-gray-50/50 p-8 lg:col-span-7 xl:col-span-8 lg:bg-white lg:p-12 flex flex-col justify-center">
            ...
            {profile?.skills && profile.skills.length > 0 && (<div>
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.skills.map((skill, index) => (<span key={index} className="inline-flex items-center rounded-xl bg-[#FFFBEB] px-4 py-2 text-sm font-bold text-yellow-800 ring-1 ring-inset ring-[#FFC629]/20">
                      {skill}
                    </span>))}
                </div>
              </div>)}
        ...
        <div className="mt-16">
          ...
          {designs.length > 0 ? (<div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
              {designs.map((design) => (<Link key={design.id} href={`/designs/${design.id}`} className="group relative">
                  ...
                  <div className="aspect-3/4 w-full overflow-hidden rounded-3xl bg-gray-200 shadow-sm transition-all duration-300 group-hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
                    <Image src={design.imageUrl} alt={design.title} fill className="h-full w-full object-cover object-center transition-transform duration-500 group-hover:scale-105"/>
                    
                    {/* Badge: Custom vs Catalog */}
                    <div className="absolute top-3 left-3">
                       {design.type === 'CUSTOM' ? (<span className="inline-flex items-center rounded-full bg-black/70 backdrop-blur-md px-3 py-1 text-xs font-bold text-white tracking-wide">
                            CUSTOM REQUEST
                          </span>) : (<span className="inline-flex items-center rounded-full bg-[#FFC629] px-3 py-1 text-xs font-bold text-black tracking-wide">
                            MADE TO ORDER
                          </span>)}
                    </div>
                  </div>
            ...
            </div>) : (<div className="rounded-3xl border-2 border-dashed border-gray-200 p-12 text-center">
