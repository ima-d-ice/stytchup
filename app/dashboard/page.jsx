    ...
    return (<div className="min-h-screen bg-gray-50 p-6 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-10">
          ...
          <Link href="/designs/add" className="bg-black text-white px-6 py-3 rounded-full font-bold hover:bg-gray-800 transition-all">
            + New Design
          </Link>
        </header>
        ...
        <div className="space-y-6">
          {orders.length === 0 ? (<div className="p-12 text-center bg-white rounded-3xl border border-dashed border-gray-200">
              <p className="text-gray-400">No active orders right now.</p>
            </div>) : (orders.map((order) => (<div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col lg:flex-row gap-8">
                
                {/* 1. Design & Buyer Info */}
                <div className="flex gap-4 lg:w-1/3">
                  <div className="relative h-20 w-20 rounded-xl overflow-hidden bg-gray-100 shrink-0">
                    {order.designSnapshot?.image ? (<Image src={order.designSnapshot.image} alt="Design" fill className="object-cover"/>) : (<div className="flex h-full w-full items-center justify-center text-xs text-gray-400">
                        No Image
                      </div>)}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="h-5 w-5 rounded-full bg-gray-200 overflow-hidden relative">
                         {order.buyer.profile?.avatarUrl && (<Image src={order.buyer.profile.avatarUrl} alt="Buyer" fill/>)}
                      </div>
                ...
                <div className="lg:w-1/3 bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  {order.status === 'AWAITING_REQUIREMENTS' ? (<div className="flex items-center justify-center h-full pb-4 text-center">
                      <p className="text-sm text-red-500 font-medium animate-pulse">
                        ⏳ Waiting for customer to submit sizes...
                      </p>
                    </div>) : order.customerMeasurements ? (<div className="grid grid-cols-2 gap-y-2 text-sm text-gray-700">
                      ...
                      {order.customerMeasurements.notes && (<p className="col-span-2 mt-2 text-xs italic text-gray-500 border-t border-gray-200 pt-2">
                          &quot;{order.customerMeasurements.notes}&quot;
                        </p>)}
                    </div>) : (<p className="text-xs text-gray-400">No measurements available</p>)}
                </div>
                ...
                <div className="lg:w-1/3 flex flex-col justify-center items-end gap-3">
                  ...
                  {/* Status Badge */}
                  <div className={`px-4 py-2 rounded-lg text-sm font-bold capitalize ${order.status === 'IN_PROGRESS' ? 'bg-yellow-100 text-yellow-800' :
                order.status === 'SHIPPED' ? 'bg-blue-100 text-blue-800' :
                  ...
                  {order.status === 'IN_PROGRESS' && (<button onClick={() => setSelectedOrderForShipping(order.id)} className="bg-black text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-gray-800 shadow-lg shadow-gray-200 transition-all">
                      Ship Item 🚚
                    </button>)}
        ...
        {/* MODAL */}
        {selectedOrderForShipping && (<ShipModal orderId={selectedOrderForShipping} onClose={() => setSelectedOrderForShipping(null)} onSuccess={() => {
